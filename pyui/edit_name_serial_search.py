...
class EditNameSerialSearchForm(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()
        self.ui = edit_name_serial.Ui_Form()
...
class Ui_Form(object):
    ...
    def edit_name_serial_click(self):
        if self.item_text != '':
            ...
            self.cur.execute("SELECT serial FROM t1 WHERE name=?", (text,))
            edit_name_serial.old_serial = self.cur.fetchone()[0]
            edit_name_serial.old_name = text
            self.name_inpt.clear()
            self.item_text = ''
            if not self.editNameSerialForm:
                self.editNameSerialForm = EditNameSerialSearchForm()
            edit_name_serial.Ui_Form.update_input_txts(edit_name_serial.s)
            self.editNameSerialForm.show()
        else:
            dialog = QMessageBox()
    ...
    def setupUi(self, Form):
        ...
        self.edit_btn.clicked.connect(self.edit_name_serial_click)
